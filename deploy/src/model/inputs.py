...
#: 14.331 — **홀수 여덟 차수**로 넓혔다 (h13·h15 를 더했다). 계측기는 15차까지 보낸다.
#  ⚠⚠ **짝수차는 안 넣는다.** 파일 13개(단독 8 + 실측 5)에서 잰 |V_h|/|V_1| 중앙:
...
#      (첫 판에서 15차수 전부를 넣었다가 짝수 7개 = **14채널을 낭비**할 뻔했다.)
#  ⚠ 이 줄을 바꾸면 RAW/FINE/WIDE 채널 수가 전부 바뀌고 **캐시와 체크포인트가
#    통째로 호환 불가**가 된다. `load_model` 이 `volt_orders` 로 하드 스톱한다.
#  ⚠ Ĝ 추정 자체에는 이득이 없다 (6차수 0.111 대 15차수 0.107 mS).
#    넓힌 이유는 **어드미턴스 특징 Y_h = I_h/V_h** 가 그 차수의 전압을 요구해서다.
VOLT_ORDERS = (1, 3, 5, 7, 9, 11, 13, 15)
...
# ── 수신기 CSV -> 원시 채널 (14.378) ─────────────────────────────────────
#: ⚠⚠ 이 둘은 `run_live.py` 와 배포 묶음의 `predictor.py` 에 **따로 적혀 있었다.**
#: 둘 다 `x = np.empty(33)` 이라 전압 고조파(33~48)를 **안 채웠다** — 그래서 실시간
#: 갈래는 채널 49 짜리 지금 모델을 원리상 못 돌린다. 한 곳으로 모은다
#: ([[verify-the-input-path-not-just-the-model]] — 도구마다 모델을 따로 지으면
#: 한 갈래만 조용히 망가진다. 이 프로젝트에서 이미 한 번 당했다).


def csv_columns(header) -> Dict[str, int]:
    """수신기 CSV 헤더 -> {열 이름: 번호}."""
    return {name: i for i, name in enumerate(header)}


def row_to_channels(row, col: Dict[str, int]) -> Optional[np.ndarray]:
    """CSV 한 행 -> **원시 49채널** 한 사이클.
    ...
    ```
      ...
      V_h    = vh_rms * (cos + i sin)(radians(vh_deg))      <- `numpy_exporter` 1b
    ```
    ⚠ `vhdeg` 는 **4차 펌웨어(2026-09-04)부터**다. 없으면 위상 0 으로 둔다 —
      `numpy_exporter` 가 `voltage_phase_available: False` 로 하는 것과 같다.
      ⚠ 그때 `arg(V_1)=0` 이 되어 **전압 고조파가 전부 실수축에 눕는다.** 모델은
      Re/Im 을 따로 받으므로 옛 펌웨어 자료는 학습 분포 밖이다. 부르는 쪽이
      `voltage_phase_available` 로 경고해야 한다.
    """
    try:
        ...
        mag = np.array([float(row[col["ih%d" % k]]) for k in range(1, N_HARM + 1)], np.float32)
        deg = np.array([float(row[col["ihdeg%d" % k]]) for k in range(1, N_HARM + 1)], np.float32)
    except (ValueError, IndexError, KeyError):
        return None
    rad = np.radians(deg)
    s = v * irms
    q = np.sign(phase) * np.sqrt(max(0.0, s * s - p * p))
    x = np.zeros(RAW_CHANNELS, np.float32)
    x[0:15] = mag * np.cos(rad)
    x[15:30] = mag * np.sin(rad)
    x[30], x[31], x[32] = p, q, v
    try:
        vm = np.array([float(row[col["vh%d" % h]]) for h in VOLT_ORDERS], np.float32)
    except (ValueError, IndexError, KeyError):
        return x                                   # 전압 고조파가 없는 옛 CSV
    try:
        vd = np.radians(np.array([float(row[col["vhdeg%d" % h]]) for h in VOLT_ORDERS],
                                 np.float32))
    except (ValueError, IndexError, KeyError):
        vd = np.zeros(len(VOLT_ORDERS), np.float32)
    x[VOLT_RE0:VOLT_IM0] = vm * np.cos(vd)
    x[VOLT_IM0:RAW_CHANNELS] = vm * np.sin(vd)
    return x


def csv_has_voltage_harmonics(col: Dict[str, int]) -> Tuple[bool, bool]:
    """(크기가 있나, 위상이 있나). 부르는 쪽이 경고를 찍는 데 쓴다."""
    m = all(("vh%d" % h) in col for h in VOLT_ORDERS)
    d = m and all(("vhdeg%d" % h) in col for h in VOLT_ORDERS)
    return m, d
