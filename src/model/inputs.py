...
FINE_CHANNELS = 52           # 36 + 추세 2 + 위상 8 + 역률·고차비 2 + 반파 1 + |I2| 1
                             #   + 다단강하 2 (12.62) + 반파 대비 1 (12.114)

#: 반파 정류 대비 채널의 자리 (12.114).
#
# **50~57 의 홀수차 크기 블록(12.80)은 은퇴시키고 그 자리를 쓴다.** 그 블록은
# 소진 목록 2번이다 — 미니PC 0.763 -> 0.658 로 반증됐고, 운영점 계보
# (`cnn_ovh`/`adapt_ovh`/`cnn_smpsf`/`adapt_smpsf`) 가 전부 `fine_channels=50`
# 이라 애초에 안 쓰고 있다.
#
# 뒤에 붙여 FINE_CHANNELS=59 로 두면 **반증된 8채널이 학습에 같이 되살아나
# 단일 변수가 깨진다** — 40행 주석이 경고하는 바로 그 함정이다. 그래서 슬롯을
# 물려받는다. 0~49 는 그대로라 옛 체크포인트는 `fine[:, :50]` 으로 계속 돈다
# (`build_fine` 산출물의 0~49 가 바이트 일치함을 확인했다).
#
# ⚠ 50~57 블록을 되살리려면 이 채널을 먼저 옮겨야 한다. 지금은 `>= 53`·`>= 58`
# 가드가 FINE_CHANNELS=51 에서 안 걸려 충돌하지 않는다.
HALFWAVE_CH = 50

#: 짝수 2차의 **절대량** 채널 (2026-09-03, 12.164.16). 오븐 FAN_LIGHT 전용 판별자다.
EVEN2_CH = 51
WIDE_CHANNELS = 12
...
#
# ⚠ **2026-09-06 계측기 교체 — 위 근거(12.72)는 옛 계측기의 것이다.** 짝수차 인공물의 뿌리는 차동 ADC 의
#   공통모드 한계였고(VINP>2.65V 에서 위쪽 피크 부풀림), 단일 입력 ADC 로 바꾼 새 계측기에는 없다:
#   순저항(포트) |I2|/|I1| 0.03%, 전압 vh2/vh1 0.03% (옛 2.6%). 남은 짝수차는 실신호다 —
#   SMPS 1.3~1.9%(도통 비대칭), 오븐 팬·조명 5.8%(12.164.16), 드라이기 약풍 반파(미재측정).
#   그래도 **True 로 둔다**: 채널을 되살리는 것은 캐시·1단계·2단계 재학습이 필요한 단일 변수 실험이고
#   (규칙 20·48), 옛 자료의 24/41·28/41 이득이 새 자료에서도 성립하는지는 아무도 안 쟀다.
#   ch50(|I2|−|I4|)·ch51(|I2|) 이 짝수차 신호의 큰 몫을 이미 나른다. 되살리려면 READ_ME_FIRST.md 의
#   재검토 목록에서 이 항목을 먼저 닫아라. 12.186.
...
def build_fine(x: np.ndarray) -> np.ndarray:
    ...
    **새 채널은 반드시 뒤에 붙인다.** 앞 38채널이 그대로 남아야 옛 체크포인트가
    ...
    # ── 반파 정류 대비 |I2| − |I4| (2026-08-27, 12.114절) ────────────────
    # 12.109.2 가 드라이기 약풍을 **다이오드 반파 정류**로 확정했다 (P 가 정확히
    # 절반, `|I2|/|I1|` 0.395, 이론 0.424). 그런데 12.77 의 짝수차 배제가 그것까지
    # 0 으로 만들고 있었다 — 저항 3종에서 **유일하게 신원이 확실한 신호**를.
    #
    # 짝수차 전체를 되살리면 12.72 의 계측 인공물이 함께 돌아온다. 그럴 필요가
    # 없다. **둘은 차수 프로파일의 모양에서 갈린다.**
    #
    #   반파 이론  |I2|:|I4|:|I6|:|I8| = 0.424 : 0.085 : 0.036 : 0.020   가파르다
    #   인공물                          평평 (12.71.2: 0.0288/0.0292/0.0290/0.0289)
    #
    # 그래서 `|I2| − |I4|` 는 평평한 성분을 상쇄하고 반파만 남긴다. 격리에서
    # (61주기 평활 후) 드라이기 약풍 0.670A vs **나머지 전 기기·전 상태 ≤0.014A**,
    # 겹침 0 이다. 핫플(101.8Ω)과 드라이기 약(겉보기 102.6Ω)은 12.112.3 이
    # 저항 정합의 유일한 모호쌍으로 남긴 것인데, 여기서 0.0001 vs 0.670 으로 갈린다.
    #
    # **비율이 아니라 절대량이다.** `|I2|/|I1|` 문턱은 복합에서 분모가 터져 죽는다
    # (12.79 가 미니PC 에서 겪은 그것). 실측 13파일에서 재면 `test_12`(|I1| 9.15A)
    # 에서 비율 형태는 발화의 72% 를, `test_10` 은 전부를 놓친다. 절대 차분은 안 그렇다.
    #
    # 실측 검증: `test_11` 정밀도 0.981 / `test_12` 1.000 (드라이기 ON 중 재현율
    # 0.196·0.279 로, 격리에서 잰 약풍 비중 31.4% 와 맞는다). SMPS 전용 `test_13`
    # 과 `test_7`·`test_8` 은 0.0% — **오탐이 없다.**
    #
    # ⚠ `ZERO_EVEN_HARMONICS` 는 True 로 둔다. 이 채널은 `EVEN_FINE_CHANNELS`
    # 목록에 없고, 드라이기 외 8종에서 수치적으로 ~0 이라 그 창들의 입력이
    # 사실상 안 바뀐다. 그래서 12.77 을 되돌리지 않고도 신호만 얻는다.
    if FINE_CHANNELS > HALFWAVE_CH:
        out[:, HALFWAVE_CH] = np.arcsinh(
            _movavg(mag[:, 1] - mag[:, 3], HALFWAVE_HALF) * CURRENT_SCALE)

    # ── |I2| 절대량 (2026-09-03, 12.164.16) ─────────────────────────────
    # 오븐 FAN_LIGHT(14.2W / 67mA)의 신원은 **h2 에만** 남아 있다. 15W 로 맞춰
    # 비교하면 h1 은 9종이 모두 64~71mA 라 크기만 말하고 신원은 말하지 않는데,
    # 비율 지문은 이렇다:
    #
    #     h2/h1   FAN_LIGHT 9.33%  |  저항 4종 0.06~0.35%  |  SMPS 3종 0.28~1.42%
    #
    # 차수별 전류 예산으로 보면 **h2 에서만 안 묻힌다** — 각 기기를 대표 통전
    # 전력으로 켰을 때 FAN_LIGHT 대비 배수가 h2 에서 포트 0.6 / 드라이기 0.7 /
    # 프로젝터 0.1 / 최악(오븐 자기 히터) 2.2 인데, h3 에서는 프로젝터 28.6,
    # 에어컨 212 다. 그래서 홀수차만 보는 지금 모델은 **조용한 창에서만** 잡았다
    # (σ(plugged)[oven] 이 배경 <60W 에서 0.973, >400W 에서 0.087).
    #
    # **인공물이 아니다 (12.164.17).** 실측 on/off 계단을 기기별로 비교하면
    # 프로젝터는 h1 이 오븐의 4배로 뛰는데 h2 는 1/4 만 움직인다:
    #     test_13 프로젝터  Δ|I1| 173.8  Δ|I2| 0.85   (h2/h1 0.5%)
    #     test_13 미니PC    Δ|I1|  82.7  Δ|I2| 1.18   (1.4%)
    #     test_11 오븐      Δ|I1|  44.4  Δ|I2| 3.33   (**7.5%**)
    #     test_12 오븐      Δ|I1|  44.7  Δ|I2| 3.11   (**7.0%**)
    # 12.72 의 레인지 전환 인공물이라면 전류가 클수록 커야 하는데 정반대다.
    # 실측 비율(7.0~7.5% vs 0.5~1.4%)이 합성 예측(9.33% vs 0.28~1.42%)과 맞는다.
    #
    # **차분(`|I2|−|I4|`)이 아니라 절대량이다.** ch50 은 반파의 가파른 차수
    # 프로파일을 노려 평평한 성분을 상쇄하는데, FAN_LIGHT 의 짝수차는 오히려
    # 평평해서(6.28/4.31/4.19) 차분이 1.97mA 로 신호를 **지운다**. 실측 계단도
    # 그렇다 (test_11 에서 3.33−3.41 = −0.08).
    #
    # ⚠ `ZERO_EVEN_HARMONICS` 는 True 로 둔다 — 이 채널은 `EVEN_FINE_CHANNELS`
    # 목록에 없다. ch50 과 같은 규약으로 **뒤에 붙인다.**
    if FINE_CHANNELS > EVEN2_CH:
        out[:, EVEN2_CH] = np.arcsinh(
            _movavg(mag[:, 1], HALFWAVE_HALF) * CURRENT_SCALE)
