...
```
  ...
  **겹침창 오븐통전%**    전이 ±20초 안 · 큰 저항 부하 · 오븐 참OFF 창에서 통전
    ⚠⚠ **이름을 고쳤다 (14.138).** 원래 "전이근처" 라고 불렀는데 그 축은
       **시간이 아니었다.** 이 모집단 905창의 27%가 포트 · 51%가 드라이기이고,
       ±20초 **밖** 138창은 **100% 핫플레이트 단독**이다. 즉 이 자는
       "전이에 가깝나" 가 아니라 **"핫플 단독이 아닌가"** 를 재고 있었다.
       라벨 전이 거리축에서 헛율은 ±2~20초 내내 **평평한 8~11%** 이고 ±20초
       밖에서 **0%** 로 떨어진다 — 감쇠가 아니라 **절벽**이라 거리 효과가 아니다.
       (`src/run_diag_stepside.py` 가 잰다. 모집단은 **안 바꿨다** — 판정 여덟과
        이어야 하므로 숫자는 그대로 유효하고 **이름만** 틀렸었다.)
  **전이밖 오븐통전%**    같은 조건, 전이 ±20초 **밖** — 실제로는 **핫플 단독 138창**이다
...
def main() -> int:
    ...
    print("  모집단: 포트창 %d창 · 겹침창 %d창 · 핫플단독 %d창 · 오븐 참ON %d창\n"
          % (M2.sum(), M5.sum(), M5o.sum(), ov.sum()))
    #: 이름은 **무엇이 문제인지** 바로 읽히게 쓴다. ★번호는 못 박은 자(★1·★2)에만 쓴다.
    hdr = ("포트창 오븐통전%", "포트창 오븐게이트%", "겹침창 오븐통전%",
           "핫플단독 오븐통전%", "오븐 재현", "오븐 헛전력 W")
