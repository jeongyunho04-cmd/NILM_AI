...
def main():
    ...
    for wh in (0.00, 0.40, 1.00):
        ...
        crit = NILMLoss(
            ...
            weights=LossWeights(harm=0.1, cons=wh),
        ).to(dev)
        if base is None:
            with torch.no_grad():
        ...
        print("   w_cons %.2f 끝" % wh)

    print("\n**검정 절반** 지수 — 몸통 얼림 · 헤드만 재적합")
    print("  %-18s %7s %8s %9s %9s %9s %9s"
          % ("기기", "창", "참값 e", "원래", "cons 0", "cons .4", "cons 1.0"))
    for a_ in RES:
        if a_ not in base:
            continue
        nn_, et, e0 = base[a_]
        cells = [rows[w].get(a_, (0, 0, float("nan")))[2] for w in (0.00, 0.40, 1.00)]
