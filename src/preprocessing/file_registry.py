...
DEVICE_FILES: Dict[str, DeviceSpec] = {
    ...
    "oven_2":            _dev("oven"),
    # ── 2026-09-09~09-10 추가 (세션 E3, 229~230V). **새 펌웨어**(96열, `vhhi17~31`).
    # 13.83.5 가 낸 구멍을 겨냥한 재녹화다 — 옛 충전기 자료는 40~70W 가 82.7% 고
    # 저·중전력(15~40W)이 총 140초·최장 18.7초·기울기 0.89W/s 인 **전이**뿐이라
    # 60초 창을 그 상태로 채울 수 없었다 (실측 배포의 충전기는 중앙 23.8W 다).
    #
    # 충전기 3. 09-09 18:26, 5.2분. 통전 78.9%, 60~80W 가 66.2% · 대기 0.81W. 사이클 최대 67.9W.
    ...
    # 기울기 0.076W/s — 정상상태다. 이것 하나로 저·중전력 창을 채울 수 있다 (13.83.7).
    # ⚠ **15~25W 는 아직 0초다.** 배터리가 27W 에서 멈췄다. 완충 직전 구간이 더 필요하다.
    ...
    # ── 자리 F (세션 F1, 2026-09-17 저녁). 시연 예정 건물이라 **배포 도메인**이다 (14.409).
    #    기존 풀은 D·E 둘뿐이라 "자리"와 "녹화"를 가를 수 없었다. 이 넷이 셋째 자리를 준다.
    # 미니PC 5. 09-17 21:25, 6.7분. V 218.3, ON 7.8W(사이클 최대 18.8), 대기 0.55W, 통전 80%,
    # ∠I1 +29.9°, |I3/I1| 85.5%. LOW 만. ⚠ 지문 추정이 시끄럽다 — 같은 녹화 반쪽끼리
    # 백색화 홀수차 거리가 **48%** 다 (충전기 5% · 빔 26%). 7W 라 그렇다.
    "minipc_5":          _dev("minipc"),
    # 충전기 7. 09-17 21:17, 6.9분. V 217.1, ON 55.4W(최대 66.0), 대기 0.52W, 통전 82%,
    # ∠I1 +4.9°, |I3/I1| 85.5%. LOW 만. 지문이 **자리 D 위에 앉는다** (D↔F 40% 대 D↔E 93%,
    # 반쪽 바닥 5%) — 전압 지문이 예측한 그대로다. 자리 간 전이의 첫 실측 확인이다.
    "laptop_charger_7":  _dev("laptop_charger"),
    # 프로젝터 8. 09-17 21:34, 7.5분. V 218.0, ON 45.0W, **램프 기동 서지 264W**, 대기 0.53W,
    # 통전 60%, ∠I1 +10.8°, |I3/I1| 83.5%. 서지 때만 HIGH. D↔F 41% (D↔E 113%).
    "beam_projector_8":  _dev("beam_projector"),
    # 전기포트 5. 09-17 20:52, 2.4분, **14번 켜고 끔**. V 219.5, 1296W, 통전 41%, HIGH.
    # R = V²/P **35.82Ω** — 자리 E 의 `electric_kettle_1` 35.60Ω 과 0.6% 차이라 **같은 주전자**다.
    # |I3/I1| 1.13% = vh3/V1 1.05% 로 순저항 거동도 같다.
    # ⚠⚠ **규칙 74 를 통과 못 한다**: ∠I−∠V(h1)이 **+2.23°** 다 (D/E 는 −0.12~+0.06°, 문턱의 37배).
    #   계측기 `phase_deg` 도 −2.230 으로 같은 말을 하고, 14번 펄스 3,609 표본에서 사분위가
    #   +2.230~+2.240 이라 **0.01° 안의 상수**다. 배제한 것: 병렬 대기 부하(바닥 3.3mA인데
    #   234mA 필요) · `h·δ` 타이밍(차수 기울기가 D/E 와 같다, −1.35 대 −1.17~−1.55 °/차수) ·
    #   PLL 기준 이동(∠I−∠V 는 차분이라 상쇄) · 배선 인덕턴스(부호가 **반대**고 0.3° 급).
    #   test_6 의 1796W 저항 구간도 +2.54° 라 **각도가 상수**(직각전류가 아니라) — 전류 채널
    #   위상오차의 꼴이다. **지문 거리에는 영향 0%** (h1 을 −2.24° 되돌려도 D↔F 가 그대로).
    #   ⇒ 가르는 실험: **같은 주전자를 D 나 E 에 다시 꽂아 30초 녹화**. 따라오면 계측기가 바뀐 것.
    # ⓘ 원본 이름 `nilm_20260917_205227.csv` 는 `COMPOSITE_EVAL_PATTERNS` 에 걸려 복합으로
    #   새므로 바꿔 등록했다 (`minipc_4` 와 같은 사유).
    "electric_kettle_5": _dev("electiric_kettle"),
}
...
SITE_OF_STEM: Dict[str, str] = {
    ...
    **{s: "E" for s in ("laptop_charger_3", "laptop_charger_4", "laptop_charger_5", "laptop_charger_6",
                        "minipc_4")},
    "test_2": "E",
    # F — 2026-09-17 저녁. 사용자가 **새 장소**라고 알려 준 자리. 전압 고조파로도 갈린다:
    # vh7 1.65%% 는 D(0.86~1.28)보다 위고 E(0.13~0.37)와는 13배 떨어져 있다.
    # ⚠ D 와는 vh7 이 1.6배뿐이라 D/E 만큼 깨끗한 틈은 아니다. 여섯 파일이 서로 붙어 있어
    #   **한 세션**인 것은 확실하다 (14.409).
    **{s: "F" for s in ("test_6", "test_7", "minipc_5", "laptop_charger_7",
                        "beam_projector_8", "electric_kettle_5")},
}
...
SITE_PROFILE: Dict[str, dict] = {
    ...
    "F": {"v_rms": 218.1, "vh3_pct": 1.01, "vh7_pct": 1.65, "z_ohm": 0.68},
}
...
SITE_SESSIONS: Dict[str, dict] = {
    ...
    # 2026-09-09 18:26 ~ 09-10 15:32. 사용자 확인: **같은 자리 같은 콘센트**라 한 세션이다
    # (날짜가 갈리지만 vh 지문이 붙어 있다 — 3파일 합쳐 vh3 3.42 p25~p75 3.39~3.79%).
    # vh7 0.37% 는 E1(0.13)·E2(0.30)보다 높지만 D(0.86~1.28%)와는 여전히 2배 이상
    # 떨어져 있어 자리 판별은 안 흔들린다. **새 펌웨어**(vhhi17~31) 첫 기기 녹화다.
    ...
    "E3": {"site": "E", "date": "2026-09-09", "v_rms": 229.5, "vh3_pct": 3.42, "vh5_pct": 1.08,
           ...
           "stems": ("laptop_charger_3", "laptop_charger_4", "laptop_charger_5", "laptop_charger_6",
                     "minipc_4")},
    # 2026-09-17 20:52~21:47. 새 자리 F. 한 저녁에 단독 넷(SMPS 3종 + 1,296W 순저항)과
    # 복합 둘을 몰아 녹화했다. Z 는 test_6 의 kW 급 계단 셋에서 쟀다 (0.63/0.80/0.68 -> 0.68Ω).
    # ⚠⚠ **순저항 h1 위상이 +2.2° 다** (D/E 는 −0.2~+0.1°). 규칙 74 를 통과 못 한다.
    #   차수 비례 기울기는 D/E 와 같으니(−1.35 대 −1.17~−1.55 °/차수) `METER_PHASE_CAL` 이
    #   어긋난 것은 아니고, h1 에만 있는 오프셋이다. 같은 값이 충전기(+1.7°)·빔(+3.7°)에도
    #   보여 **부하가 아니라 자리/계측 공통**이다 (14.409). 원인 미상 — 쓰기 전에 규명해라.
    "F1": {"site": "F", "date": "2026-09-17", "v_rms": 218.1, "vh3_pct": 1.01, "vh5_pct": 1.93,
           "vh7_pct": 1.65, "z_ohm": 0.68,
           "stems": ("test_6", "test_7", "minipc_5", "laptop_charger_7", "beam_projector_8",
                     "electric_kettle_5")},
}
