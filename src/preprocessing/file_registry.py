...
[장소]
옛 문서의 장소 A/B/C 지문(vrms 218~239V, vh3 3.9~10V)은 **옛 계측기의 전압 채널 인공물**
(h3 덧셈 2.6% of V1, 12.185.25)이 섞인 값이라 더 이상 쓰지 않는다. 새 계측기 자료는 전부
사용자 자리(옛 표기 'C')에서 찍었고, 같은 콘센트에서 시간대에 따라 이렇게 갈린다 (2026-09-05/06):
...
즉 "장소" 축은 이제 **세션(시간대)** 축이다. `site_of()` 는 새 파일에 전부 'C' 를 준다.
...
DEVICE_FILES: Dict[str, DeviceSpec] = {
    # 프로젝터 ×2. 09-06 00:40 / 01:10 (심야 무리, 231V / 229V). ON 46.2 / 46.1W (사이클 최대 50.9 / 48.0),
    ...
    # 전기포트. 09-06 00:30 (심야, 228V). 1458W, HIGH 61%, on/off 여러 번. R = V²/P 35.61Ω (p5~p95 35.18~35.69).
    # 순저항 검정(규칙 74): ihdeg1 −0.05° (HIGH), |I3/I1| 3.23% = vh3/V1 3.25% (옛 계측기의 h3 바닥이 없다).
    "electric_kettle_1": _dev("electiric_kettle"),
    # 선풍기. 09-06 01:46 (심야, 229V). 9.7분, 1/2/3단 22.9 / 30.8 / 39.0W (옛 상태 문턱 10/27/36 그대로 맞는다),
    # ∠I1 +30 / +21 / +4°, PF 0.86 / 0.93 / 1.00, |I3|/|I1| 0.09 / 0.07 / 0.03, 짝수차 0.002. LOW 만. `.cal2` 꼬리 없음.
    "fan_1":             _dev("fan"),
    # 드라이기. 09-06 01:57 (심야, 227V). 8.2분, 강 965.8W(HIGH 100%, R 53.22Ω, |I2| 0.000, ∠I1 −0.11° — 순저항) /
    ...
    # 핫플레이트. 09-05 23:00 (저녁, 215V). 456.5W 통전 41%, 릴레이 주기 부하, LOW 만. R 100.83Ω. ihdeg1 +0.05°.
    "hotplate_1":        _dev("hotplate"),
    # 충전기. 09-05 21:26 (저녁, 216V). 16분, 65W 정속이 대부분(60~70W 87%), 20~60W 테이퍼 일부. 사이클 최대 68.2W.
    # ⚠ 72W 정전류 구간·돌입이 없다 — `postproc.ABSORB_CAP_W` 84.6 은 옛 녹화의 사이클 최대라 이 파일로 못 낮춘다.
    "laptop_charger_1":  _dev("laptop_charger"),
    # 미니PC. 09-05 22:17 (저녁, 218V). 27분, IDLE 10~13W 28% / 부하 13~30W 64% / 대기 3.4W. 사이클 최대 29.5W.
    "minipc_1":          _dev("minipc"),
    # 에어컨. 09-06 02:11 (심야, 228V). 27.8분, 대기 6.8W/117mA∠+76°(꽂힌 대기, 용량성) / 송풍 10~30W / 냉방 80~640W
    ...
    # 충전기 2. 09-06 02:42 (심야, 230V). 4.2분, 10~70W 테이퍼·정속 골고루, 꼬리 20초 1.47W/6.4mA. 규칙 2 를 지킨 첫 충전기 녹화.
    "laptop_charger_2":  _dev("laptop_charger"),
    # 오븐. 09-05 21:47 (저녁, 216V). 18분, 히터 1101W(30.8%) / 팬·조명 16.2W(61.6%) / 대기 2.4W. R(히터) 40.09Ω.
