...
from src.model.inputs import (FINE_CYCLES, FINE_LAYOUT, LEGACY_FINE_CHANNELS, WIDE_CHANNELS,
                             ZERO_EVEN_HARMONICS,
                              ...
                              TARGET_LOOKAHEAD)
from src.model.build import (build_model, check_input_convention,  # noqa: F401
                             comb_over_of, model_kwargs)
...
#: 14.378 — 정본은 `src/model/build.comb_over_of` 다. 이름을 남겨 두는 건
#: 부르는 곳이 여럿이라서다 (`run_gate_pintot` · `run_plot_real`).
_comb_over_of = comb_over_of
...
def load_model(ckpt_path: str, dev: str, weights: bool = True, mask: bool = True,
    ...
    #: ★ 14.378 — 규약 검사와 모델 짓기는 **`src/model/build.py` 가 정본**이다.
    #  배포 묶음의 `predictor.py` 가 같은 함수를 부른다 — 예전에는 거기가 인자를
    #  다섯 개만 넘겨 **조합 머리가 통째로 빠진 모델**을 지었다.
    check_input_convention(ck, ckpt_path)
    model, apps = build_model(ck, dev, weights=weights, mask=mask,
                              state_power_init=state_power_init,
                              proj_from=proj_from, ckpt_path=ckpt_path)
