...
#: ⚠⚠⚠ 14.401 — **표를 고쳤다. 앞의 값은 두 군데가 틀렸다** (§58).
#:
#: ① **`U(−a, a)` 의 표준편차는 `a/√3` 이다.** 잰 σ 를 `a` 에 그대로 넣어서
#:    실제 주입 std 가 **0.577σ** 였다. `a = σ·√3` 이어야 한다
#: ② **σ 를 나쁜 추정기로 쟀다.** 옛 σ(0.98/1.78/4.17)는 h13·h15 를 적합에 넣은
#:    값인데 그 차수는 **세션 배경이 기기 전류보다 크다**(13.84.38) — 회전이 아니라
#:    공통 가산항이다. §53 이 고친 추정기(h3~h11 · 크기 가중 · h1 제외)로 다시 재면
#:    σ = 0.67 / 2.73 / 2.75 다
#:
#: ⇒ 그 둘을 고치면 표가 이렇게 된다 (옛 √3 표 1.69/3.09/7.22 와도 다르다):
#: ```
#:   기기      녹화  R²    녹화간 σ   **a = σ·√3**   옛 a    덜 흔든 배수
#:   미니PC     4   0.81     0.67       **1.16**     0.94      x1.24
#:   충전기     6   0.86     2.73       **4.73**     2.13      **x2.22**
#:   빔         3   0.98     2.75       **4.76**     3.66      x1.30
#: ```
#: ★ 왜 이것이 중요한가 — `cnn_pjit`(v50) 이 **깎은 만큼 졌다**:
#: ```
#:   미니PC  4.0 -> 0.94 (4.3배 깎음)  실측 AUC **−0.0378** (t −4.5)  ✗
#:   충전기  4.0 -> 2.13 (1.9배)                 −0.0027
#:   빔      4.0 -> 3.66 (1.1배)       실측 AUC **+0.0165** (t +3.5)  ○
#:   ⇒ 완벽한 단조다. 덜 흔들면 캐시 위상이 **너무 깨끗해져** 모델이
#:     전이 안 되는 위상을 외운다 (§54: 녹화 회전은 추론에서 관측 불가)
#: ```
#: ⚠ 빔은 3.66 -> 4.76 이라 **처음으로 더 흔드는** 기기다. 위험 지점이라 자를 따로 둔다.
PHASE_JITTER_DEG_MEASURED: Dict[str, float] = {
    "minipc":          1.16,   # 녹화 4 · R² 0.81 · σ 0.67 x√3
    "laptop_charger":  4.73,   # 녹화 6 · R² 0.86 · σ 2.73 x√3
    "beam_projector":  4.76,   # 녹화 3 · R² 0.98 · σ 2.75 x√3  <- 처음으로 **더** 흔든다
}
#: 옛 표 (14.389~14.400). 관문이 "바뀌었나" 를 이걸로 확인한다.
PHASE_JITTER_DEG_V50: Dict[str, float] = {
    "minipc": 0.94, "laptop_charger": 2.13, "beam_projector": 3.66,
}
