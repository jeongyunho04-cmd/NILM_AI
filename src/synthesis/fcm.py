...
class DeviceModel12:
    ...
    **CT 위상 상수는 넣지 않는다 (설계 13.3).** README_v12 가 남긴 "2Hz 피처에 CT +1.4° 반영" 을 2Hz 녹화로 검증했다:
    `measured=True`(RC τ=60µs) 출력이 심야 229V 녹화(laptop_charger_2·beam_projector_1/2)와 h1~h9 복소오차 1.3~5% 로
    맞고, +1.4°/h 를 더하면 ±0.2%p 안에서 오히려 나빠진다. 원시 FFT 페이저(측정 V1 기준)와 2Hz 페이저(펌웨어)는
    −1.0~−1.7°·h 만큼 다른데, 그것이 곧 RC 지연이라 이 출력이 이미 2Hz 영역이다. 저녁 216V 녹화는 R 을 어떻게 잡아도
    15~30% 어긋난다 — 원시 스냅샷이 전부 심야라 저녁 텍스처에서 모델이 검증되지 않았다 (13.3.4).
    """
