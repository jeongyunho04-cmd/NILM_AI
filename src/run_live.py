...
def main() -> int:
    ...
    # 2026-09-04: `adapt_zi_s0` -> **`adapt_ac_s0`** (12.175 채택, 사용자 결정).
    # 현직은 **장소 B 가 생기기 전에 적응한 모델**이라 새 장소에서 안 돈다.
    # 같은 자(병합 채점, 3시드 중앙)로 재면:
    #
    #             장소A F1  유령W  |  장소B F1   잔차W   유령W
    #   adapt_zi_s0  0.920   3.96  |   0.714  **146.1**  **133.3**
    #   adapt_ac_s0  0.930   1.27  |   0.915     15.3      10.7
    #
    # 장소 B 잔차가 146 -> 15W, 유령이 133 -> 11W 다. 장소 A 도 지지 않는다
    # (0.920 -> 0.930, 유령 3.96 -> 1.27).
    #
    # 이 모델의 계보 (12.164~12.175):
    #   반파 채널 ch50 + **|I2| 채널 ch51**(12.164.16) -> FINE_CHANNELS 52
    #   `gt_plugged[oven]` = 동작 세션 중 (12.164) + `--standby-operating session`
    #   `--sp-curves` 부하 의존 서명 (12.166)
    #   **에어컨 꽂힘 상한 0.10** (12.174) — 합성 상시 바닥을 실측에 맞춘다
    #   2단계: w_res 0.1 / w_swap 2.0 / harm_offset / smps_boost 4.0
    #
    # ⚠ **시드마다 실패 자리가 다르다** (규칙 50). 셋 다 남겨 뒀으니 시연
    #   리허설에서 보고 바꿔도 된다. `s0` 은 **검출이 가장 좋은 대신 유령이
    #   가장 크다** — 사용자 판단으로 일단 이것으로 둔다.
    #
    #      장소B F1 (파일별)                    장소B 유령W          장소A 약점
    #   s0  0.938/0.943/0.859/0.908 (최고)   **test_15 33.75** (에어컨 30.96)  —
    #   s1  0.915/0.929/0.821/0.901          0.57~1.96 (최저)   **포트 0.502**
    #   s2  0.910/0.928/0.807/0.893          1.40~4.58          프로젝터 0.769
    #
    #   `s0` 의 유령은 **test_15 한 파일에 몰려 있다** (나머지 셋은 1.4~7.0W).
    #   시연에서 "없는 에어컨이 31W" 가 보이면 `results/adapt_ac_s1.pt` 로
    #   바꾸면 된다 — 유령은 깨끗해지고 장소 B 검출이 0.912 -> 0.891 로 진다.
    #   ⚠ 재학습으로는 못 고른다. 세 `.pt` 파일을 그대로 써야 한다.
    # ⚠ 미니PC IDLE 전력은 여전히 1.3W 다 (참값 ~12W, 12.175.2). 검출은 되고
    #   전력만 작다. 다음 수는 `L_pref` 와 차분 항이다.
    #
    # ── 2026-09-04 (오후) `adapt_ac_s0` -> **`adapt_ph5_s0`** (사용자 결정, 12.183.3) ──
    # 12.179~12.180 이 그 유령의 뿌리를 찾았다: 장소 B 의 전압 고조파가 SMPS 의
    # h9~h15 전류 위상을 돌려 놓고, 1단계가 그 창에서 드라이기 강풍을 못 본다.
    # `cnn_ph5` 는 캐시 증강에 **h5 이상 위상 지터**(60°/σ0.5, `--dither-min-order 5`)
    # 를 넣어 그 위상을 믿지 않게 학습한 1단계이고 (12.182~12.183), 2단계 레시피는
    # `adapt_ac` 와 같다. `--site-transfer` 없음 — 체크포인트의 `site_transfer` 가 None.
    #
    #                  장소A F1   장소B F1   s0 유령(test_15)    시드
    #   adapt_ac_s0     0.930     0.915     33.75W 없는 에어컨   갈림
    #   adapt_ph5_s0    0.903     0.903      1.1W               셋이 같다
    #
    # **교환이다.** 장소 A 0.03 을 내주고 "포트+드라이기 강풍이 겹치면 없는 에어컨
    # 558W" 를 없앴다. 대가는 SMPS 판별 일부 — 12.36 의 최상위 판별자 |I9|/|I3| 가
    # h9 에 살고 장소 B 가 돌리는 것도 h9 라, 흔들면 판별을 잃고 지키면 장소를 못
    # 덮는다 (규칙 73). 미니PC 신원(h11~h15, 기기별 장소 의존)은 어느 판에서도 안
    # 생겼다 — 충전기가 켜지면 3~7W 를 미니PC 로 낸다.
    # 시드 셋이 같은 답을 내므로 시연에서 시드를 고를 일이 없다.
    ap.add_argument("--ckpt", default="results/adapt_ph5_s0.pt")
