...
KNOWN_DEBT = {
    ...
    "harm_smps": "14.375 — 1단계에 SMPS 전용 고조파 항을 뚫었다 (저항 몫을 Ĥ·V_h 로 뺀다). adapt 은 아직. ⚠ 이 팔이 채택되면 **즉시 갚아야 한다** — 안 갚으면 adapt 이 합쳐진 잔차 하나로 돌아가 SMPS 신호가 저항(관측의 95~100%)에 다시 묻힌다",
    "harm_smps_min_order": "위와 한 쌍",
    "smps_sel": "위와 한 쌍 (기기 이름 선택자 — 풀 없이 `SMPS_GROUP` 으로 지으면 된다)",
