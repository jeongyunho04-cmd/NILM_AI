...
# ⚠ **2026-09-06 계측기 교체 — 이 표는 옛 계측기(차동 ADC) 격리 녹화의 값이다. 재계산 전에는 채점에 쓰지 마라.**
#   새 자료를 전처리한 뒤 `python -m src.run_power_check --recompute-ref` 로 다시 낸다. 새 계측기에서 잠정으로
#   본 것 (원본 CSV, 60초 전부-ON 창): 프로젝터 중앙 46.2/45.9 (p5 44.8, p95 47.3, 창 55개).
#   포트·핫플은 한 번에 60초 넘게 켜진 구간이 없어 창이 0개, 오븐은 1개 — 이 표의 저항 3종 값은
#   **전압에도 걸려 있다** (P = V²/R: 포트 1458W@228V 대 옛 1277W@214V). 저항 부하의 참값은 상수가
#   아니라 `V²/RESISTIVE_OHM` 으로 내는 것이 맞다 — 재계산 때 같이 고칠 것. 12.186.
...
EXCLUDED_REASON: Dict[str, str] = {
    ...
    "minipc": "폭/중앙 1.162 (8.2~22.7W). 유휴/작업 3.3배 (12.32.1). "
              "⚠ IDLE 만 보면 0.065 다 — `REFERENCE_W_STEMWISE` 참조 (12.159)",
