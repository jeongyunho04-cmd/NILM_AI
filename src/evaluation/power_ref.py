...
# **2026-09-06 재계산 완료** (설계 13.9). 새 계측기 격리 녹화로 `--recompute-ref` 를 돌린 값이다.
#   저항 3종은 `REFERENCE_V` 와 짝이라 `_ref_at` 이 `P = V²/R` 로 보정한다 — 격리 녹화 전압이 옛 자료와
#   14V 까지 달라(포트 213 -> 227.7V) 상수로 두면 그대로 틀린다. 등가저항 `V²/W` 로 검산하면:
#       포트 35.60Ω   핫플 101.84Ω   오븐 41.10Ω
#   ON 구간 전부의 중앙값으로 직접 잰 값은 35.59 / 101.17 / 40.15Ω 다 (READ_ME_FIRST §3 은 35.61 / 100.83 / 40.09).
#   포트는 0.03%, 핫플은 0.7% 로 맞고 **오븐만 2.4% 높다** — 오븐은 서모스탯이라 60초 창이 통전/휴지 전이를
#   걸치면 평균이 내려가고(창 4개뿐이다) 그만큼 등가저항이 커 보인다. 오븐 참값을 더 좁히려면 히터가 연속으로
#   60초 넘게 도는 구간이 여러 개 있는 녹화가 필요하다.
#   **프로젝터가 표에서 빠졌다** — 폭/중앙 0.142 로 문턱(0.10)을 넘는다 (아래 EXCLUDED_REASON).
REFERENCE_W: Dict[str, tuple] = {
    "electiric_kettle": (1456.5, 1455.3, 1459.1),   # 녹화 1개, 창 10, 폭/중앙 0.003
    "hotplate": (451.8, 451.5, 452.2),              # 녹화 1개, 창  8, 폭/중앙 0.001
    "oven": (1073.9, 1066.4, 1082.0),               # 녹화 1개, 창  4, 폭/중앙 0.015
    # 13.50 — **되살렸다.** 위 78행이 "폭/중앙 0.142 라 뺐다, 상태를 갈라야 쓴다"
    # 고 적었는데, 그 처방대로 `is_on` 구간의 **60초 창 중앙값**으로 다시 재니
    #     _1(E1) 44.75 · _2(E1) 44.69 · _3(D2) 45.26 W
    #     p5 44.46 / 중앙 44.85 / p95 45.72 -> 폭/중앙 **0.028** (문턱 0.10)
    # 두 장소에서 1.3% 안이다. 옛 0.142 는 더 잔 창 기준이라 켠 직후가 섞였다.
    # 이 키가 없어서 `run_adapt --w-pref` 가 기본 `--pref-apps beam_projector`
    # 에서 **아무것도 안 걸고 있었다** (12.144.2 가 82.8W -> 47.5W 로 쟀던 항).
    # ⚠ `is_on` 에 WARMUP_COOLDOWN 이 4.8% (4.6W) 섞인다. LAMP_ON 95.2% 는 44.89W.
    "beam_projector": (44.9, 44.5, 45.7),           # 녹화 3개, 창 23, 폭/중앙 0.028
}

#: **특정 파일에서만** 참값을 아는 기기 (2026-09-03, 12.159).
#
# 사용자가 알려준 측정 조건: **`test_14`~`test_18` 을 녹화할 때 미니PC 는 마우스가
# 고장나 있었다.** 그래서 작업을 시키지 못하고 켜놓기만 했고, 켜져 있는 동안
# 전부 `IDLE` 상태다.
#
# 아래 `EXCLUDED_REASON` 이 미니PC 를 뺀 이유는 *"폭/중앙 1.162 (8.2~22.7W).
# 유휴/작업 3.3배"* 인데, 그것은 두 상태를 **합쳐서** 잰 값이다. 갈라 보면:
#
#     IDLE          9.54 / 10.18 / 9.90 W     **폭/중앙 0.065**   <- 채택 문턱 통과
#     ACTIVE_LOAD  16.27 / 18.75 / 21.64 W      폭/중앙 0.286
#
# 신호로 교차 확인된다 — 정밀화 라벨의 미니PC 계단이 test_15 +14.7, test_16 +10.5,
# test_18 +10.7/+10.1W 로 전부 IDLE 범위다 (ACTIVE 면 +16~22W 여야 한다).
#
# ⚠ **전역으로 쓰면 안 된다.** 장소 A 의 `test_13` 에는 +27.3W 계단이 있어
#   ACTIVE 가 섞인다. 그래서 파일 목록과 함께 둔다 — 규칙 14 는 "안 잰 것을
#   측정처럼 쓰지 마라" 이고, 여기서는 **어디서 쟀는지까지** 적어야 한다.
# ⚠ **2026-09-06: 비었다.** 위 항목은 전부 옛 계측기 복합 녹화(`test_14`~`test_18`, 삭제됨)의 것이다.
#   새 자료의 미니PC 단독 녹화(`minipc_1`)는 유휴/작업이 한 파일에 섞여 있어 폭/중앙 0.948 이고,
#   상태를 갈라도 "그 파일에서만 성립" 을 뒷받침할 복합 녹화가 아직 없다. 새 복합에서 미니PC 가
#   한 상태로만 도는 구간이 확인되면 그때 채운다. 규칙 14 — 안 잰 것을 측정처럼 쓰지 않는다.
REFERENCE_W_STEMWISE: Dict[str, tuple] = {
    # 기기: (참값 튜플, 그 값이 성립하는 파일)
}

#: 표에 **안 넣은** 기기와 그 이유 (규칙 14 — 안 잰 것을 측정처럼 쓰지 않는다).
#: 폭/중앙이 0.10 을 넘으면 상태가 여럿이라 단일 참값을 못 쓴다.
#: 2026-09-06 재계산 값 (새 계측기 격리 녹화).
EXCLUDED_REASON: Dict[str, str] = {
    "air_conditioner": "폭/중앙 1.673 (16.1~622.7W). 인버터라 연속 가변",
    "minipc": "폭/중앙 0.948 (9.8~25.2W). 유휴/작업 (창 90개)",
    "laptop_charger": "폭/중앙 0.316 (45.3~64.0W). 고속/트리클",
    "fan": "폭/중앙 0.494 (23.0~37.5W). 풍량 단계",
    "hair_dryer": "폭/중앙 0.289 (574~778W). 강/약",
    # 13.50: **다시 채택했다** (위 REFERENCE_W 주석). 60초 창 중앙값으로 재면 0.028 이다.
    #   옛 줄을 지우지 않고 남긴다 — 왜 뺐다가 다시 넣었는지가 기록이다.
    # "beam_projector": "폭/중앙 0.142 (38.9~45.2W). **새로 제외** — 옛 표에서는 0.077 로 "
    #                   "들어 있었다. 중앙 44.5W 인데 p5 가 38.9W 라 켠 직후/저광량 구간이 "
    #                   "섞인다. 상태를 갈라야 쓴다",
}
