...
⚠ **위 넷은 계정 소유자에 대한 의무다. 아래는 우리가 스스로 건 제한이라 출처를 구분해 적는다.**

### 자체 제한 — 출처별

| 제한 | 출처 | 상태 |
|---|---|---|
| 동시 **3노드** | **사용자 지시 2026-09-09** | ⚠ **2026-09-18 사용자가 풀었다** (*"3노드 제한 상관없어"*) |
| `composite_eval` 업로드 금지 | **내가 적은 것** (9fe99ff, 2026-09-13) | **2026-09-18 폐지** ↓ |

**`composite_eval` 업로드 금지는 폐지했다 (2026-09-18).**
원래 논거는 *"학습에는 안 들어가므로 거기 둘 이유가 없다"* 는 **필요성**이었지 기밀이
아니었다. 그런데 필요가 생겼다 — 팔이 셋씩 늘면서 실측 채점이 이 노트북에서
**판당 6.4초 x 파일 6 x 판 15 = 10분**이 되고, 그게 판정을 붙잡는다.
사용자 확인: *"무슨 기밀정보도 아니고"* (2026-09-18).

⚠ 그래도 **상주시키지는 않는다** — 55MB 이고 공용 계정이므로 채점이 끝나면 지운다.
  `run_judge_arms` 를 HPC 에서 돌릴 때의 규약:
```bash
tar cf - processed_data/composite_eval processed_data/real_events.json   | ssh gate1_External 'cd ~/NILM_AI && tar xf -'
ssh gate1_External 'cd ~/NILM_AI && sbatch patches/judge_arms.sbatch'   # 채점 뒤 스스로 지운다
```
⚠ 그리고 `is_sealed` 는 그대로다 — 봉인 파일은 어디서도 안 읽는다.

**우리 것은 `~/NILM_AI` 하나뿐이다.** `rm -rf ~/NILM_AI` 로 전부 되돌아간다.

### ~~자체 제한: 동시 3노드~~ — **2026-09-18 풀렸다**

2026-09-09 사용자 지시로 걸었다가 2026-09-18 사용자가 풀었다 (*"3노드 제한 상관없어"*).
그 전까지의 근거는 *"허락은 받았지만 조심히 쓴다"* 였다.
QOS 상한은 `node=10 / cpu=640 / gpu=12` 다.

⚠ **그래도 큰 작업은 사용자 동의를 먼저 받는다** — 이건 안 풀렸다.
  실측: 987056(캐시 v52)이 8토막을 6노드에, 987069/70/81(학습 3팔)이 15판을 GPU 에 올렸다.
